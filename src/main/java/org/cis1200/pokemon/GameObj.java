...
public abstract class GameObj {
    ...
    public boolean intersectsCenter(GameObj that) {
        int cx1 = this.getPx() + this.getWidth() / 2;
        int cy1 = this.getPy() + this.getHeight() / 2;

        int cx2 = that.getPx() + that.getWidth() / 2;
        int cy2 = that.getPy() + that.getHeight() / 2;

        double dx = cx2 - cx1;
        double dy = cy2 - cy1;
        double dist = Math.sqrt(dx * dx + dy * dy);

        double hitRadius = Math.min(this.getWidth(), this.getHeight()) / 4.0;

        return dist < hitRadius;
    }

    public boolean intersectsCenter(Rectangle rect) {
        return rect.intersects(getCenterHitbox());
    }
...
//        int h = getHeight() / 2;
//
//        int cx = getPx() + getWidth() / 2 - w / 2;
//        int cy = getPy() + getHeight() / 2 - h / 2;
//
...
//    }
    ...
    public Rectangle getCenterHitbox() {
        ...
        int startX = getPx() + getWidth() / 5;
        int startY = getPy() + getHeight() / 3;
